...
def _structure(tmp_path, second=("ATOM", "ALA")):
...
class TestTheCheck:
    def _check(self, tmp_path, reference, second=("ATOM", "ALA")):
        pdb = _structure(tmp_path, second)
    ...
    def test_a_selenomethionine_the_rebuild_drops_is_caught(self, tmp_path):
        # MSE is written as HETATM, and gen_ali does not rebuild HETATM residues,
        # so the chain comes out one residue short: 3I2V chain A does exactly this.
