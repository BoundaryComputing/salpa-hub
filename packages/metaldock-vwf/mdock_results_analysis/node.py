...
class MdockResultsAnalysis(Node):
    ...
    OPTIONS = {
        ...
        "output_dir": FolderParameter(
            "Output Directory",
            docstring="Working dir for the analysis summary JSON.",
        ),
    ...
    def execute(self, predecessor_data, flow_vars):
        ...
        try:
            ...
            protein_ref = flow_vars["protein_pdb"].get_value() or carried.get(
                "cleaned_pdb"
            )
            if not protein_ref:
                raise NodeException(
                    "setup", "No protein PDB (param or predecessor cleaned_pdb)."
                )
            ...
            if not pose_xyz_paths:
                raise NodeException(
                    "setup", "No pose_xyz_paths found in predecessor data."
                )
            ...
            reference_xyz = (
                Path(self.resolve_path(ref_xyz_ref)) if ref_xyz_ref else None
            )

            stream_log(
                "Parsing energies + residue contacts...",
            ...
            # Poses come in the order AutoDock ran them, not by score, so the best
            # one is named by index rather than assumed to be the first.
            best = analysis.get("best_pose_index")
            summary = {
                "case_name": case_name,
            ...
            best_energy = (
                summary["binding_energies"][best] if best is not None else None
            )
            ...
            result.success = True
            result.message = (
                f"Best ΔG {best_energy:.2f} kcal/mol (pose {best + 1}), "
                f"{len(residues[best])} contacting residues"
                if best is not None
